...
// const getHouses = (arr) => arr.map(family => family.house);
...
const totalCharacters = (arr) => {
  let numberOfChars = [];
  arr.forEach(item => {
    numberOfChars.push(item.name);
    if (item.spouse) {
      numberOfChars.push(item.spouse);
    }
    item.children.forEach(kid => numberOfChars.push(kid));
  })

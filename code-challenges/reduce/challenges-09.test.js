...
const reversedString = (str) => {
  let arr = str.split('');
  return arr.reduce((accumulator, value, idx) => value + accumulator, '')
};
...
const calculateAverage = (arr) => {
  let average = arr.reduce((accumulator, value, idx) => {
    accumulator.count++;
...
const countPrimeNumbers = (arr) => {
  let allPrimes = arr.reduce((accumulator, value, idx) => {
    if (isPrime(value)) {
      return ++accumulator;
    } else {
      return accumulator;
...
const extractChildren = (arr) => {
  let filterChars = arr.filter(item => {
    return /[a]/.test(item.name);
  })
  let allKids = filterChars.reduce((accumulator, value, idx) => {
